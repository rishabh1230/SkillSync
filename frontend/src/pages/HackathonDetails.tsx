...
import {
  ...
  UserPlus, ChevronDown, ChevronUp, X, Check
} from 'lucide-react';
...
const HackathonDetails: React.FC = () => {
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '400px' }}>
        <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: 'linear' }}
          style={{ width: '40px', height: '40px', borderRadius: '50%', border: '3px solid rgba(6,182,212,0.2)', borderTop: '3px solid #06b6d4' }} />
      </div>
    );
  }
  ...
  const renderRegistration = () => {
    if (isHost) {
      return (
        <div style={{ padding: '12px', background: 'rgba(6,182,212,0.07)', borderRadius: '10px', border: '1px solid rgba(6,182,212,0.2)', textAlign: 'center' }}>
          <p style={{ color: '#06b6d4', fontWeight: 600, fontSize: '0.88rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
      ...
      return (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '10px' }}>
            ...
            <p style={{ color: '#10b981', fontWeight: 700, fontSize: '0.9rem' }}>Registered with a team!</p>
          </div>
          <button onClick={() => navigate(`/teams/${registeredTeamId}/submit`)}
            style={{ width: '100%', padding: '10px', background: '#10b981', color: '#fff', border: 'none', borderRadius: '8px', fontWeight: 700, cursor: 'pointer', fontSize: '0.88rem' }}>
            Submit Project
          </button>
        </motion.div>
      );
      ...
      return (
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '6px' }}>
            <CheckCircle size={16} color="#10b981" />
            <p style={{ color: '#10b981', fontWeight: 700, fontSize: '0.9rem' }}>Registered as Solo!</p>
          </div>
      ...
      return (
        <div style={{ display: 'flex', gap: '8px' }}>
          <button
            ...
            style={{ flex: 1, padding: '12px 8px', background: 'linear-gradient(135deg, #06b6d4, #8b5cf6)', color: '#fff', border: 'none', borderRadius: '10px', fontWeight: 700, fontSize: '0.82rem', cursor: 'pointer', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '5px' }}
          >
            <Crown size={18} />
            Create a Team
          </button>
    ...
    if (step === 'team_form') {
      return (
        <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }}
          style={{ padding: '16px', background: 'rgba(6,182,212,0.05)', border: '1px solid rgba(6,182,212,0.25)', borderRadius: '12px' }}>
          <h4 style={{ color: '#fff', fontWeight: 700, marginBottom: '14px', fontSize: '0.9rem', display: 'flex', alignItems: 'center', gap: '6px' }}>
            <Crown size={15} color="#06b6d4" /> Create Your Team
          </h4>
          ...
          <input type="text" placeholder="Enter team name..." value={teamName} autoFocus
            ...
            style={{ width: '100%', padding: '10px 13px', background: 'rgba(0,0,0,0.4)', border: `1px solid ${regError ? '#f87171' : 'rgba(6,182,212,0.35)'}`, borderRadius: '8px', color: '#fff', fontSize: '0.88rem', outline: 'none', marginBottom: regError ? '6px' : '12px', boxSizing: 'border-box' }}
          />
          ...
          <div style={{ display: 'flex', gap: '8px' }}>
            ...
            <button onClick={handleCreateTeam} disabled={registering || !teamName.trim()}
              style={{ flex: 2, padding: '9px', background: registering || !teamName.trim() ? 'rgba(6,182,212,0.3)' : '#06b6d4', color: '#fff', border: 'none', borderRadius: '8px', cursor: registering || !teamName.trim() ? 'not-allowed' : 'pointer', fontWeight: 700, fontSize: '0.88rem' }}>
  ...
  return (
    <div style={{ padding: '24px', maxWidth: '1200px', margin: '0 auto' }}>
      ...
      {/* Banner */}
      <div style={{ height: '260px', borderRadius: '20px 20px 0 0', background: hackathon.bannerImage ? `url(${hackathon.bannerImage}) center/cover` : 'linear-gradient(135deg, #0f172a 0%, #1e1a3a 50%, #0f172a 100%)', position: 'relative', overflow: 'hidden', border: '1px solid rgba(255,255,255,0.06)', borderBottom: 'none' }}>
        ...
        <div style={{ position: 'absolute', bottom: '24px', left: '28px', right: '28px', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
          <div>
            <span style={{ display: 'inline-block', padding: '4px 12px', background: isCompleted ? 'rgba(16,185,129,0.2)' : 'rgba(6,182,212,0.2)', color: isCompleted ? '#10b981' : '#06b6d4', borderRadius: '20px', fontSize: '0.7rem', fontWeight: 700, border: `1px solid ${isCompleted ? '#10b981' : '#06b6d4'}`, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: '8px' }}>
              {hackathon.status}
            </span>
            <h1 style={{ fontSize: '2rem', fontWeight: 800, color: '#fff', textShadow: '0 2px 8px rgba(0,0,0,0.5)', margin: 0 }}>{hackathon.title}</h1>
          </div>
          {hackathon.prizePool && (
      ...
      <div style={{ display: 'flex', gap: '2px', background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderTop: 'none', borderBottomLeftRadius: '16px', borderBottomRightRadius: '16px', padding: '6px', marginBottom: '28px' }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            style={{ flex: 1, padding: '10px 16px', background: activeTab === tab.id ? 'rgba(6,182,212,0.12)' : 'transparent', color: activeTab === tab.id ? '#06b6d4' : 'var(--text-muted)', border: activeTab === tab.id ? '1px solid rgba(6,182,212,0.25)' : '1px solid transparent', borderRadius: '10px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '7px', fontWeight: activeTab === tab.id ? 600 : 400, fontSize: '0.88rem', transition: 'all 0.2s' }}>
            {tab.icon} {tab.label}
          </button>
        ))}
      </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 360px', gap: '28px', alignItems: 'start' }}>
              <div>
                ...
                <div style={{ background: 'rgba(255,255,255,0.03)', borderRadius: '14px', padding: '22px', border: '1px solid rgba(255,255,255,0.07)', marginBottom: '20px' }}>
                  ...
                  <p style={{ color: 'var(--text-secondary)', lineHeight: 1.7, fontSize: '0.92rem' }}>{hackathon.fullDescription}</p>
                </div>
                  ...
                  <div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                      {hackathon.problemStatements.map((p: any, i: number) => (
                        <div key={p.id || i} style={{ padding: '16px', background: 'rgba(6,182,212,0.04)', borderRadius: '12px', border: '1px solid rgba(6,182,212,0.12)' }}>
                          <h4 style={{ color: '#06b6d4', marginBottom: '6px', fontWeight: 600 }}>{p.title}</h4>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                <div style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '22px' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '13px', marginBottom: '22px' }}>
                    {[
                      { icon: <Calendar size={15} />, color: '#06b6d4', bg: 'rgba(6,182,212,0.1)', label: 'Starts', value: new Date(hackathon.hackathonStartDate).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) },
                      ...
                      { icon: <Users size={15} />, color: '#10b981', bg: 'rgba(16,185,129,0.1)', label: 'Team Size', value: `${hackathon.minTeamSize}–${hackathon.maxTeamSize} members` },
              ...
              <>
                ...
                <div style={{ marginBottom: '36px' }}>
                  <h2 style={{ color: '#fff', fontSize: '1.15rem', fontWeight: 700, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px' }}>
                    <Users size={18} color="#06b6d4" /> Teams <span style={{ color: 'var(--text-muted)', fontWeight: 400, fontSize: '0.9rem' }}>({teams.length})</span>
                  </h2>
                              ...
                              <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', marginBottom: '14px' }}>
                                {team.members?.map((m: any) => (
                                  <div key={m.id} style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                    <div style={{ width: '26px', height: '26px', borderRadius: '50%', background: m.role === 'LEADER' ? 'linear-gradient(135deg, #06b6d4, #8b5cf6)' : 'rgba(255,255,255,0.08)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                                    ...
                                    <span style={{ color: m.role === 'LEADER' ? '#fff' : 'var(--text-secondary)', fontSize: '0.83rem' }}>
                                      {m.userId.slice(0, 8)}...
                                      {m.role === 'LEADER' && <span style={{ marginLeft: '6px', fontSize: '0.68rem', color: '#06b6d4', fontWeight: 600 }}>Leader</span>}
                                    </span>
                                  </div>
                                ))}
                              </div>
                                ...
                                isRequestingThis ? (
                                  <div>
                                    <textarea
                                      ...
                                      style={{ width: '100%', padding: '8px 12px', background: 'rgba(0,0,0,0.4)', border: '1px solid rgba(6,182,212,0.3)', borderRadius: '8px', color: '#fff', fontSize: '0.83rem', outline: 'none', resize: 'none', height: '60px', marginBottom: '8px', boxSizing: 'border-box' }}
                                    />
                                    <div style={{ display: 'flex', gap: '6px' }}>
                                      ...
                                      <button onClick={() => handleSendJoinRequest(team.id)} disabled={requestingTeamId === team.id}
                                        style={{ flex: 2, padding: '8px', background: requestingTeamId === team.id ? 'rgba(6,182,212,0.3)' : '#06b6d4', color: '#fff', border: 'none', borderRadius: '7px', cursor: 'pointer', fontWeight: 700, fontSize: '0.8rem' }}>
                                  ...
                                  <button onClick={() => setRequestingTo(team.id)}
                                    style={{ width: '100%', padding: '8px', background: 'rgba(6,182,212,0.08)', border: '1px solid rgba(6,182,212,0.2)', borderRadius: '8px', color: '#06b6d4', cursor: 'pointer', fontWeight: 600, fontSize: '0.83rem', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '6px' }}>
                                            ...
                                            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: req.message ? '6px' : '0' }}>
                                              ...
                                              <div style={{ display: 'flex', gap: '6px' }}>
                                                <button onClick={() => handleRespondToRequest(req.id, team.id, true)} disabled={respondingId === req.id}
                                                  style={{ padding: '4px 10px', background: 'rgba(16,185,129,0.15)', border: '1px solid rgba(16,185,129,0.4)', borderRadius: '6px', color: '#10b981', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 700, display: 'flex', alignItems: 'center', gap: '4px' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(240px, 1fr))', gap: '12px' }}>
                      {soloParticipants.map((reg: any, i: number) => (
                        <motion.div key={reg.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                          ...
                          <div style={{ width: '36px', height: '36px', borderRadius: '50%', background: 'linear-gradient(135deg, #8b5cf6, #06b6d4)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
              ...
              <>
                <div style={{ marginBottom: '20px', padding: '14px 18px', borderRadius: '12px', background: projectsData.isCompleted ? 'rgba(16,185,129,0.08)' : 'rgba(251,191,36,0.08)', border: `1px solid ${projectsData.isCompleted ? 'rgba(16,185,129,0.2)' : 'rgba(251,191,36,0.2)'}`, display: 'flex', alignItems: 'center', gap: '10px' }}>
                  {projectsData.isCompleted
                    ? <><CheckCircle size={16} color="#10b981" /><span style={{ color: '#10b981', fontSize: '0.88rem', fontWeight: 600 }}>Hackathon complete — all projects are publicly visible, ordered by submission time.</span></>
                    : projectsData.isHost
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
                    {projectsData.submissions.map((sub: any, i: number) => (
                      <motion.div key={sub.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.04 }}
                        style={{ background: 'rgba(255,255,255,0.03)', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '14px', padding: '18px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '16px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flex: 1, minWidth: 0 }}>
                          <div style={{ width: '36px', height: '36px', borderRadius: '10px', background: i === 0 ? 'linear-gradient(135deg, #fbbf24, #f59e0b)' : i === 1 ? 'linear-gradient(135deg, #9ca3af, #6b7280)' : i === 2 ? 'linear-gradient(135deg, #d97706, #b45309)' : 'rgba(255,255,255,0.06)', display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 800, color: i < 3 ? '#fff' : 'var(--text-muted)', fontSize: '0.9rem', flexShrink: 0 }}>
                            #{i + 1}
                          </div>
                          <div>
                            <p style={{ color: '#fff', fontWeight: 600, fontSize: '0.95rem', marginBottom: '3px' }}>{sub.team?.teamName || 'Unknown Team'}</p>
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0 }}>
                          {projectsData.isHost && !projectsData.isCompleted && (
                            <button onClick={() => handleToggleVisibility(sub.id)} disabled={togglingId === sub.id} title={sub.isVisible ? 'Hide from participants' : 'Show to participants'}
                              style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 13px', background: sub.isVisible ? 'rgba(16,185,129,0.12)' : 'rgba(255,255,255,0.05)', border: `1px solid ${sub.isVisible ? 'rgba(16,185,129,0.35)' : 'rgba(255,255,255,0.1)'}`, borderRadius: '8px', color: sub.isVisible ? '#10b981' : 'var(--text-muted)', cursor: togglingId === sub.id ? 'not-allowed' : 'pointer', fontSize: '0.78rem', fontWeight: 600, opacity: togglingId === sub.id ? 0.5 : 1 }}>
                              {sub.isVisible ? <><Eye size={13} /> Visible</> : <><EyeOff size={13} /> Hidden</>}
                            </button>
                          )}
                          <button onClick={() => navigate(`/projects/${sub.projectId}`)}
                            style={{ display: 'flex', alignItems: 'center', gap: '6px', padding: '7px 13px', background: 'rgba(6,182,212,0.1)', border: '1px solid rgba(6,182,212,0.25)', borderRadius: '8px', color: '#06b6d4', cursor: 'pointer', fontSize: '0.78rem', fontWeight: 600 }}>
